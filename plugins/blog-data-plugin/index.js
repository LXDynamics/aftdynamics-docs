...
/**
 * AFT Dynamics Blog Data Plugin for Docusaurus
 * 
  ...
  return {
    ...
    async allContentLoaded({ allContent, actions }) {
      ...
      try {
        ...
        if (!blogContent) {
          logger.warn('[blog-data-plugin] Blog plugin content not found. Ensure @docusaurus/plugin-content-blog is enabled.');
          setGlobalData({
            blogData: {
              posts: [],
              totalCount: 0,
          ...
          return {
            ...
            metadata: {
              ...
              authors: (metadata.authors || []).map(author => ({
                ...
                key: author.key || author.name?.toLowerCase().replace(/\s+/g, '-') || 'unknown',
                title: author.title,
                url: author.url,
        ...
        const tags = Object.values(postsByTag).sort((a, b) => 
        ...
        logger.info(`[blog-data-plugin] Processed ${blogData.totalCount} blog posts with ${tags.length} unique tags.`);
        ...
        authors: [${post.metadata.authors.map(author => `
          {
            ...
            key: '${escapeString(author.key)}',${author.title ? `
            title: '${escapeString(author.title)}',` : ''}${author.url ? `
            url: '${escapeString(author.url)}',` : ''}${author.imageURL ? `
